...
class HomeView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final navProvider =
        context.watch<NavigationBarProvider>(); // بتعرض الداتا << watch

    return Scaffold(
      appBar: AppBar(
        title: Center(
          child: Text(navProvider.getCurrentTitle(context)[navProvider.currentIndex]),
        ),
        actions: [
          InkWell(
            onTap: () {
      ...
      bottomNavigationBar: NavigationBar(
        ...
        onDestinationSelected: (index) {
          context.read<NavigationBarProvider>().changeCurrentIndex(
            index,
          ); // بستخدمها مع الفانكشن اللى بتغير الداتا << read
        },
      ),
    );
  }
